...
```mermaid
graph TD
    Item1["1. Fix Follow-Mode Gesture Logic<br><i>TrailMap.native.tsx</i>"] --> Item2["2. Harden Production APK Asset Copy<br><i>TrailMap.native.tsx</i>"]
    Item2 --> Item3["3. Implement PMTiles Magic Header Check<br><i>TrailMap.native.tsx</i>"]
...
### 1. Fix Camera Follow-Mode Gesture Disconnect
- **File:** [TrailMap.native.tsx:L187-203](file:///d:/thrail_app/src/features/Map/TrailMap.native.tsx#L187-L203)
- **Action:** Set `setIsFollowing(false)` on any touch gesture (`event.properties.isUserInteraction === true`), removing the `!zoomChanged` barrier. Initialize `lastCenterRef` on first fix.
- **Why #1:** Eliminates the most severe interactive bug where pinch-zooming is overridden by camera snapping back every 2 seconds.

### 2. Harden Asset Resolution for Release APKs & Remove Dead HTTP Fallback
- **File:** [TrailMap.native.tsx:L130-155](file:///d:/thrail_app/src/features/Map/TrailMap.native.tsx#L130-L155)
- **Action:** Remove `FileSystem.downloadAsync(asset.uri, fileUri)` fallback. In production release bundles, copy `asset.localUri` directly using `FileSystem.copyAsync`. If unresolvable, fail immediately to `loadState = "error"` with an actionable retry button.
- **Why #2:** Prevents fresh production installations from failing on launch with black maps and broken download loops.

### 3. Add 7-Byte PMTiles Magic Header Integrity Check
- **File:** [TrailMap.native.tsx:L120-128](file:///d:/thrail_app/src/features/Map/TrailMap.native.tsx#L120-L128)
- **Action:** Replace `fileInfo.size > 18_000_000` with a header verification reading 7 bytes via `FileSystem.readAsStringAsync(fileUri, { length: 7 })` and verifying `"PMTiles"`. Delete and re-copy if header is missing or corrupted.
- **Why #3:** Guarantees self-healing if an app is killed mid-copy on first launch or if storage writes were interrupted.
