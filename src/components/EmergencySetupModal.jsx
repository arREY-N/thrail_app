            ...
            <KeyboardAvoidingView style={styles.modalContainer} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
                ...
                <Animated.View style={[
                    ...
                    <View style={styles.headerRow}>
                        <CustomText variant="h2" style={styles.headerTitle}>{mode === 'unified' ? "Edit Contacts" : "Emergency Setup"}</CustomText>
                        <TouchableOpacity onPress={handleCloseOrSkip} style={styles.closeBtn} disabled={isSaving}>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                        ...
                        {mode === 'unified' && (
                            <View style={styles.section}>
                                ...
                                <CustomText style={styles.sectionSubtitle}>For your guide to reach you during this hike.</CustomText>
                                <CustomTextInput label="Phone Number" placeholder="9XX XXX XXXX" prefix="+63" type="phone" value={myPhone} keyboardType="number-pad" onChangeText={setMyPhone} maxLength={12} />
                            </View>
                        )}

                        <View style={[styles.section, { borderTopWidth: mode === 'unified' ? 1 : 0, borderColor: Colors.GRAY_ULTRALIGHT, paddingTop: mode === 'unified' ? 16 : 0}]}>
                            <CustomText style={styles.sectionTitle}>Emergency Contact</CustomText>
                            <CustomText style={styles.sectionSubtitle}>Link a Thrail user to unlock the automated SOS Group Chat.</CustomText>
                            ...
                            {showDropdown && searchResults.length > 0 && (
                                <View style={styles.dropdown}>
                                    {searchResults.map((user) => (
                                        <TouchableOpacity key={user.id} style={styles.dropdownItem} onPress={() => handleSelectUser(user)}>
                                            <View style={styles.dropdownAvatar}><CustomIcon library="Feather" name="user" size={16} color={Colors.PRIMARY} /></View>
                                            <View>
                                                <CustomText style={styles.dropdownName}>{user.firstname} {user.lastname}</CustomText>
                                                <CustomText style={styles.dropdownEmail}>{user.email}</CustomText>
                                            </View>
                                        </TouchableOpacity>
                                    ))}
                                </View>
                            ...
                            <View style={styles.inputSpacing}>
                                <CustomTextInput label="Contact Name" placeholder="Maria Dela Cruz" value={contactName} onChangeText={setContactName} />
                            </View>
                            
                            <View style={styles.inputSpacing}>
                                <CustomTextInput label="Contact Phone Number" placeholder="9XX XXX XXXX" prefix="+63" type="phone" value={contactPhone} keyboardType="number-pad" onChangeText={setContactPhone} maxLength={12} />
                            </View>
                        </View>
                    </ScrollView>
...
const styles = StyleSheet.create({
    ...
    dropdownName: { fontSize: 15, color: Colors.TEXT_PRIMARY, fontWeight: 'bold' },
    dropdownEmail: { fontSize: 12, color: Colors.TEXT_SECONDARY, marginTop: 2 },
