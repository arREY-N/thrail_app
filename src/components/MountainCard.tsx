...
const getRouteIconInfo = (routeType: string) => {
    ...
    if (route.includes("out") || route.includes("back") || route.includes("return")) {
    ...
    return (
        ...
        >
            <View style={styles.imageContainer}>
                ...
                {offersCount > 0 && (
                    <View style={[styles.glassPill, styles.topRightPosition, { backgroundColor: Colors.PRIMARY }]}>
                        <View style={styles.badgeSection}>
                            <CustomIcon library="Ionicons" name="pricetag" size={12} color={Colors.WHITE} />
                            <CustomText variant="caption" style={[styles.badgeText, { color: Colors.WHITE, fontWeight: 'bold' }]}>
            ...
            <View style={[
                ...
                <StatItem label="Distance" value={displayLength} icon="map-outline" lib="Ionicons" isNarrow={isNarrow} />
                {!isNarrow && <View style={styles.verticalDivider} />}
                <StatItem label="Elevation" value={displayElev} icon="trending-up" lib="Feather" isNarrow={isNarrow} />
                {!isNarrow && <View style={styles.verticalDivider} />}
                <StatItem label="Route" value={displayRoute} icon={routeIconInfo.icon} lib={routeIconInfo.lib} isNarrow={isNarrow} />
            </View>
        </TouchableOpacity>
    );
};
