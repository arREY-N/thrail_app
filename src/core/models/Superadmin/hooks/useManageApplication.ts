...
export function useManageApplication(params: UseManageApplicationParams = { role: 'user' }): IUseManageApplication {
    ...
    const onApproveApplication = async () => {
        try {
            ...
            const approved = newApplication({
                ...application,
                status: 'approved'
            })

            const business = businessFromApplication(approved);

            const applicationId = approved.id;
    ...
    const onRejectApplication = async () => {
        ...
        try {
            ...
            const rejected = newApplication({
                ...application,
                status: 'rejected'
            })

            await rejectApplication(rejected);
            router.back();
        } catch (error) {
