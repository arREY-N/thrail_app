 ...
 */
export const trailMapAssets: Record<string, any> = {
 ...
 */
export const fallbackMapAsset = require("@/src/assets/trail-maps/placeholder-map.png");
 ...
 * 
 * @param {string} trailId - The document ID of the trail.
 * @param {string} [trailName] - Optional name of the trail/mountain.
 * @returns {any} Resolved static map image asset (or fallback asset if not matched).
 */
export function getStaticMapAsset(trailId: string, trailName?: string): any {
  // 1. Direct match on ID key
  if (trailId && trailMapAssets[trailId]) {
